  ...
  export async function authenticateHelper(
    ...
    options?: {
      ...
      const payload = jwt.verify(
        ...
        if (options?.permitIfSelf && !currentUser._id.equals(options.permitIfSelf.id) && !options.permitIfSelf.requiredRoleCanOverride) {
      ...
      function sanitizeToken(authorization: string | undefined) {
        ...
        return authorization?.length === 2 ? authParts[1] : authParts[0]
      }